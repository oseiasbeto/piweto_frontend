...
// Importa a função `jwtDecode` para decodificar tokens JWT.
...
export function useUsers() { 
    ...
    // Variável para armazenar o identificador do timeout de renovação do token.
    ...
    const refreshToken = async (session_id) => {
        try {
            loading.value = true;
            await api.post("/users/refresh-access-token", { session_id })
                .then(res => {
                   ...
                   // scheduleTokenRefresh(accessToken); // Agenda a próxima renovação do token.
                })
                .finally(() => {
                    loading.value = false;
                });
    ...
    // Agenda a renovação do token automaticamente antes de expirar.
