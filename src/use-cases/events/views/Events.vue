        ...
        <div class="p-0 lg:p-5 pb-32 lg:pb-36">
            ...
            <div class="data">
                ...
                <div class="flex bg-white rounded-md flex-col flex-wrap shadow-[0_2px_10px_0_rgba(0,0,0,0.05)]">
                    ...
                    <div class="border-b border-[#dfe0df] lg:border-none">
                        <div class="lg:hidden px-4 p-4">
                            <button @click="showFilters = !showFilters"
                                class="w-auto flex items-center gap-2 text-[#0097ff] font-semibold py-2 text-xs min-w-[100px] text-left">
                                ...
                                <span class="flex items-center gap-1">
                                    <p>{{ showFilters ? 'Ocultar filtros' : 'Ver filtros' }}</p>
                        ...
                        <div :class="[
                            ...
                            <div
                                ...
                                <div class="lg:max-w-80 w-full">
                                    <p class="hidden lg:block text-brand-gray-500 font-semibold py-2 text-xs">Buscar
                                        pelo
                                        nome do
                                        evento</p>
                                ...
                                <div class="lg:max-w-60 w-full">
                                    <p class="hidden lg:block text-brand-gray-500 font-semibold py-2 text-xs">
                                        Compartilhamento</p>
                                    <Listbox v-model="filterByStaffs">
                                        <div class="relative">
                                            <ListboxButton v-slot="{ open }"
                                                class="flex h-[40px] w-full text-brand-gray-500 items-center text-xs px-3.5 p-2 overflow-hidden border border-[#dfe0df] rounded-[3px] bg-white focus:outline-none">
                    ...
                    <div class="hidden lg:block p-4">
                        <table class="w-full text-xs text-left">
                            <thead @click="fetchStaffs"
                                class="text-[11px] cursor-pointer font-bold border-[1.5px] border-[#eaeaec] text-brand-gray-500 uppercase bg-[#fbfbfb]">
                                <tr>
                                    <th class="px-4 min-h-[40px] noorder py-3">
                                        Status
                            ...
                            <tbody v-if="!loadingEvents">
                                <tr v-if="validEvents?.length" v-for="eventWrapper in validEvents"
                                    ...
                                    class="bg-white group cursor-pointer border-b border-[#dfe0df] hover:bg-[#F5F5F6] transition-colors">
                                    ...
                                    <td class="px-4 py-2 text-nowrap text-brand-gray-500">
                                    ...
                                    <td class="px-4 py-2 flex items-center justify-end gap-2 text-brand-gray-500">
                                        <button @click.stop
                                        ...
                                        <div class="w-6 h-6">
                                            <router-link @click.stop class="w-full h-full"
                                                ...
                                                <button
                                                    ...
                                                    class="w-full h-full flex text-[#0097ff] items-center justify-center rounded-ful">
                                                    <svg xmlns="http://www.w3.org/2000/svg"
                                ...
                                <tr class="border-b border-[#dfe0df]" v-if="!validEvents.length">
                                    <td class="px-4 py-[15px] text-brand-gray-500 text-nowrap">Nenhum evento encontrado
                                    </td>
                                </tr>
                            </tbody>

                            <tbody v-else>
                                <tr v-for="i in validEvents?.length || 6" :key="i"
                                    class="border-b border-[#dfe0df] animate-pulse">

                                    <!-- Status -->
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            <div class="w-[12px] h-[12px] rounded-full bg-[#dfe0df]"></div>
                                            <div class="h-3 w-16 bg-[#dfe0df] rounded"></div>
                                        </div>
                                    </td>

                                    <!-- Evento -->
                                    <td class="px-4 py-3">
                                        <div class="h-3 w-32 bg-[#dfe0df] rounded"></div>
                                    </td>
                                    ...
                                    <!-- Onde -->
                                    <td class="px-4 py-3">
                                        <div class="h-3 w-28 bg-[#dfe0df] rounded"></div>
                                    </td>

                                    <!-- Ingressos -->
                                    <td class="px-4 py-3">
                                        <div class="h-5 w-[130px] bg-[#dfe0df] rounded-lg"></div>
                                    </td>

                                    <!-- Ações -->
                                    <td class="px-4 py-3 flex justify-end gap-2">
                                        <div class="h-6 w-20 bg-[#dfe0df] rounded-full"></div>
                                        <div class="h-6 w-6 bg-[#dfe0df] rounded"></div>
                                        <div class="h-6 w-6 bg-[#dfe0df] rounded"></div>
                                    </td>
                    ...
                    <div v-if="!loadingEvents">
                        ...
                        <ul class="flex flex-col lg:hidden items-center" v-if="validEvents?.length">
                            <li class="w-full py-[10px] px-[20px] border-b-[5px] border-[#eee]"
                                ...
                                <div class="flex py-2 items-center w-full justify-between">
                                    <p class="text-sm mb-1 font-normal" style="color: rgb(25, 31, 40);">
                                        {{ truncateText(eventWrapper.event.name || "Evento", 30) }}
                                    </p>
                                    <!-- MENU -->
                                    <Menu as="div" class="relative">
                                        ...
                                        <transition @click.stop enter-active-class="transition duration-100 ease-out"
                                            ...
                                            <MenuItems v-if="openMenuId === eventWrapper.event._id" static
                                                ...
                                                <!-- Seta -->
                                                <div
                                                    class="absolute -top-2 right-3 w-4 h-4 bg-white rotate-45 border-l border-t border-black/5">
                                                </div>
                                                ...
                                                <router-link :to="`/eventos/${eventWrapper.event._id}`" :class="[
                                                    active ? 'bg-gray-100' : '',
                                                    'flex w-full items-center gap-3 px-4 py-2 text-sm text-brand-gray-500 transition'
                                                ]">
                                                ...
                                                <a :href="`/evento/${eventWrapper.event.slug}`" target="_blank" :class="[
                                                    active ? 'bg-gray-100' : '',
                                                    'flex w-full items-center gap-3 px-4 py-2 pb-2.5 text-sm text-brand-gray-500 transition'
                                                ]">
                                ...
                                <div class="flex py-2 text-xs justify-between items-center">
                                    ...
                                    <span class="flex text-[#50525f] items-center">
                                        <svg class="mr-2 shrink-0" xmlns="http://www.w3.org/2000/svg" width="32"
                        ...
                        <!-- Mensagem para quando não houver eventos -->
                        <div class="px-5 text-brand-gray-500 py-6 lg:hidden" v-else>
                            <p>Nenhum evento foi encontrado.</p>
                        </div>
                    </div>
                    <div v-else>
                        <ul class="flex flex-col lg:hidden items-center">

                            <li v-for="i in 5" :key="i"
                                class="w-full py-[10px] px-[20px] border-b-[5px] border-[#eee] animate-pulse">
                    ...
                    <div v-if="validEvents?.length" class="w-full">
                        <div class="flex flex-col sm:flex-row items-center justify-between py-4 lg:pt-0 px-4">
                            <!-- Informação de resultados -->
                            <div class="flex items-center gap-2 mb-4 sm:mb-0">
                                <p class="text-xs flex-1 text-brand-gray-500">
                                    Mostrando
                            ...
                            <div class="flex items-center space-x-2">
                                <button @click="handlePrevPage" :disabled="!events?.metadata?.hasPrevPage"
                                    class="px-4 py-2 text-xs font-medium text-brand-gray-500 bg-white border border-[#dfe0df] rounded-[3px] hover:bg-[#f1f1f1] focus:outline-none focus:ring-offset-2 transition-colors duration-200 disabled:opacity-50"
                                    :class="{ 'opacity-50': !events?.metadata?.hasPrevPage }">
                                    Anterior
                                </button>
                                ...
                                <Listbox v-model="limit">
                                    <div class="relative">
                                        <ListboxButton
                                            class="text-left w-full px-4 flex items-center py-2 text-xs font-medium text-brand-gray-500 bg-white border border-[#dfe0df] rounded-[3px] focus:outline-none transition-colors duration-200">
                                        ...
                                        <transition leave-active-class="transition duration-100 ease-in"
                                            ...
                                            <ListboxOptions
                                                class="absolute z-[100] mt-1 max-h-60 w-full overflow-auto rounded-sm bg-white text-xs shadow-lg focus:outline-none">

                                                <ListboxOption v-for="value in [10, 15, 20, 30]" :key="value"
                                                    :value="value" v-slot="{ active, selected }" as="template">
                                                    ...
                                                    <div :class="[
                                                        selected
                                                            ...
                                                            : active
                                                                ? 'bg-[#f1f1f1] text-brand-gray-500'
                                                                : 'text-brand-gray-500',
                                                        'cursor-pointer select-none py-1.5 px-4'
                                                    ]">
                                ...
                                <button @click="handleNextPage" :disabled="!events?.metadata?.hasNextPage"
                                    class="px-4 py-2 text-xs font-medium text-brand-gray-500 bg-white border border-[#dfe0df] rounded-[3px] hover:bg-[#f1f1f1] focus:outline-none focus:ring-offset-2 transition-colors duration-200 disabled:opacity-50"
                                    :class="{ 'opacity-50': !events?.metadata?.hasNextPage }">
                                    Próximo
                                </button>
                            </div>
                        </div>
...
const optionStatus = [
    ...
    { name: 'Apenas os próximos eventos', value: 'n' },
    { name: 'Eventos publicados', value: 'a' },
    { name: 'Eventos em análise', value: 'p' },
...
const optionStaffs = [
    { name: 'Todos eventos', value: null },
    { name: 'Apenas meus eventos', value: 'y' },
    { name: 'Compartilhado comigo', value: 'n' }
]
...
const isEventEnded = (eventDate) => {
    if (!eventDate) return false; // Se não houver data, considera ativo
    const now = new Date();
...
function statusColor(status) {
    switch (status) {
        ...
        case "p":
            return "bg-[#ff6a00] text-[#ff6a00]"
        case "d":
            return "bg-[#fdbd39] text-[#fdbd39]"
        case "r":
            return "bg-red-500 text-red-500"
        default:
            return "bg-[#cbcbcf] text-[#85868f]"
    }
}
...
const applyFilters = () => {
    ...
    // status
    status.value = filterByStatus.value?.value || null

    // staff (depende da tua API)
    staffType.value = filterByStaffs.value?.value || null

    fetchStaffs()
}

const clearFilters = () => {
    filterByStatus.value = optionStatus[0]
    filterByStaffs.value = optionStaffs[0]
...
// dispara automaticamente quando o limit mudar
watch(limit, async () => {
    await fetchStaffs();
})
</script>
