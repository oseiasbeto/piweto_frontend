<template>
  <div>
    <div class="relative select-none h-14 w-full" :class="{ 'pointer-events-none': props.disabled }">
      <!-- Input -->
      <input :autocomplete="props.type === 'password' ? 'new-password' : 'off'"
        :type="showPassword ? 'text' : props.type" :id="props.label"
        class="peer w-full h-full bg-transparent pt-3 px-3 text-sm border border-border rounded-[4px] focus:border-brand-primary focus:border-2 outline-none transition-all text-gray-700 placeholder-transparent peer-valid:pt-4 pr-10"
        :placeholder="props.title" :class="{ 'border-red-500 focus:border-red-500': props.error.show }"
      ...
      <label :for="props.label" class="absolute pointer-events-none left-3 top-4 text-gray-500 text-base transition-all 
          ...
          peer-focus:top-2 peer-focus:text-xs peer-focus:text-brand-primary 
